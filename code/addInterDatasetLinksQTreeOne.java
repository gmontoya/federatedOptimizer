...
class addInterDatasetLinksQTreeOne {

    public static void main(String[] args) {

        String qTreeFile1 = args[0];
        String qTreeFile2 = args[1];
        String cpsFile1 = args[2];
        String cpsFile2 = args[3];
        QTreeOne qTree1 = readQTreeOne(qTreeFile1);
        QTreeOne qTree2 = readQTreeOne(qTreeFile2);
        HashMap<String, HashMap<Integer, HashMap<Integer, Integer>>> cps1 = qTree1.getOverlap(qTree2); 
        writeCPS(cps1, cpsFile1);
        cps1.clear();
        HashMap<String, HashMap<Integer, HashMap<Integer, Integer>>> cps2 = qTree2.getOverlap(qTree1);
...
*/
    ...
    public static QTreeOne readQTreeOne(String file) {
        QTreeOne qt = null;
        try {
            ObjectInputStream in = new ObjectInputStream(
                new BufferedInputStream(new FileInputStream(file)));

            qt = (QTreeOne) in.readObject();
            in.close();
        } catch (Exception e) {
            System.err.println("Problems reading file: "+file);
            e.printStackTrace();
